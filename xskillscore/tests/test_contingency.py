...
def test_Contingency_da(forecast,observation,category_edges_a,category_edges_b, dim, type):
    if 'ds' in type:
        name='var'
    ...
    cont_table = Contingency(forecast,observation,category_edges_a,category_edges_b, dim=dim)
    assert cont_table
    # test against sklearn.metrics.confusion_matrix
    if type == 'da' and dim=='time':
        def logical(ds):
            return ds < .5
        ...
        for lon in forecast.lon:
            for lat in forecast.lat:
                ...
                print(int(lon),int(lat),sklearn_cont_table_1d, expected)
                assert sklearn_cont_table_1d == expected
