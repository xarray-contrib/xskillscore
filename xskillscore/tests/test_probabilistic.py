...
def test_crps_ensemble_dask(o_dask, f_dask):
...
def test_crps_ensemble_weighted(o, f, weights):
    dim = ['lon', 'lat']
    actual = crps_ensemble(o, f, dim=dim, weights=weights)
    assert_only_dim_reduced(dim, actual, o)


def test_crps_gaussian_dask(o_dask, f_dask):
...
def test_crps_gaussian_dask_b_int(o_dask):
    mu = 0
    sig = 1
    actual = crps_gaussian(o_dask, mu, sig)
...
def test_crps_quadrature_dask(o_dask):
    cdf_or_dist = norm
...
def test_crps_quadrature_args(o_dask, f_dask):
...
@pytest.mark.parametrize('dim', DIMS)
def test_crps_quadrature_dim(o, f, dim):
    cdf_or_dist = norm
    actual = crps_quadrature(o, cdf_or_dist, dim=dim)
    assert_only_dim_reduced(dim, actual, o)


def test_threshold_brier_score_dask(o_dask, f_dask):
    threshold = 0.5
...
def test_threshold_brier_score_multiple_thresholds_xr(o, f):
...
def test_threshold_brier_score_multiple_thresholds_dask(o_dask, f_dask):
...
def test_brier_score_dask(o_dask, f_dask):
    actual = brier_score((o_dask > 0.5), (f_dask > 0.5).mean('member'))
    assert actual.chunks is not None
